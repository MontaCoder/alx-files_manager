...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    if (parentId !== 0) {
      ...
      parentFile = await filesCollection.findOne({ _id: parentObjectId, userId: user._id });
    ...
    if (type === 'folder') {
      try {
        ...
        return response.status(201).json({ id: result.insertedId, ...fileData });
      } catch (error) {
        return response.status(500).json({ error: 'Error creating folder' });
      }
    } else {
